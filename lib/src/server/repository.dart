...
import 'package:talk/src/models/account_deactivate.dart';
import 'package:talk/src/models/get_config_model.dart';
import 'package:talk/src/models/meeting_history_model.dart';
import 'package:talk/src/models/meeting_mode.dart';
import 'package:talk/src/models/password_reset_model.dart';
import 'package:talk/src/models/privacy_policy_model.dart';
import 'package:talk/src/models/user_model.dart';
import 'package:talk/src/utils/validators.dart';
import '../../config.dart';
import 'dart:io';
...
class Repository {
  ...
  Future<AuthUser> getLoginAuthUser(String email, String password) async {
    ...
    try {
      final response =
          await dio.post("${Config.baseUrl}/api/v100/login", data: formData);
  ...
  Future<AuthUser> getRegistrationAuthUser(
    ...
    try {
      final response =
          await dio.post("${Config.baseUrl}/api/v100/signup", data: formData);
      AuthUser user = AuthUser.fromJson(response.data);
      if (user.status == 'success') {
        return user;
      } else {
  ...
  Future<AuthUser> getFirebaseAuthUser({
    ...
    try {
      final response = await dio
          .post("${Config.baseUrl}/api/v100/firebase_auth", data: formData);
      AuthUser user = AuthUser.fromJson(response.data);
      if (user.status == 'success') {
        return user;
      } else {
  ...
  Future<AuthUser> updateUserProfile(AuthUser user, File image) async {
    ...
    try {
      final response = await dio
          .post("${Config.baseUrl}/api/v100/update_profile", data: formData);
  ...
  Future<List<MeetingHistoryModel>> meetingHistory(
      String userId, String pageIndex) async {
    dio.options.headers = {"API-KEY": Config.apiKey};
    try {
      final response = await dio.get(
          "${Config.baseUrl}api/v100/meeting_history_by_user_id?user_id=$userId&page=$pageIndex");
      if (response.statusCode == 200) {
        List<MeetingHistoryModel> meetingHistoryList =
            MeetingHistoryList.fromJson(response.data).meetingHistoryList;
  ...
  //Getting Meeting History by ID
  static Future<List<MeetingHistoryModel>> meetingHistoryPagination(
      String userId, String pageIndex) async {
    Dio dio = Dio();
    dio.options.headers = {"API-KEY": Config.apiKey};
    try {
  ...
  Future<AccountDeactivate> accountDeactivate(
    ...
    try {
      final response = await dio.post(
          "${Config.baseUrl}/api/v100/deactivate_account/",
          data: formData);
  ...
  Future<PrivacyPolicyModel> privacyPolicyResponse() async {
    ...
    try {
      final response =
          await dio.get("${Config.baseUrl}api/v100/privacy_policy");
      if (response.statusCode == 200) {
        PrivacyPolicyModel meetingHistoryList =
  ...
  Future<PasswordResetModel> passResetResponse({String email}) async {
    ...
    try {
      final response = await dio
          .post("${Config.baseUrl}/api/v100/password_reset/", data: formData);
